    ...
    <ul class="list-group ">
        @foreach ($mercados as $mercado)
            <li class="list-group-item d-flex bd-highlight" name="{{ $mercado->nome_mercado }}">
                ...
                {{-- <form action="{{ route('mercado.edit'), $mercado->id}}" method="GET" class="bd-highlight ">
                    @csrf
                ...
                <form action="{{ route('mercado.destroy', $mercado->id) }}" method="POST" class="bd-highlight ms-2">
                    @csrf
                    @method('DELETE')
